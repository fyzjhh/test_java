...
public class LZ77 {
	 ...
	 * 
	 * @param infile
	 *            the name of the file to uncompress - automatically appends a
	 *            ".lz77" extension to the supplied filename
		...
		while (st.nextToken() != StreamTokenizer.TT_EOF) {
			...
			case StreamTokenizer.TT_NUMBER:
				...
				if (st.ttype == StreamTokenizer.TT_WORD) {
					...
					// therefore the first number read was actually part of a
					// word
	 ...
	 * 
	 * @param infile
	 *            the name of the file to compress. Automatically appends a
	 *            ".lz77" extension to infile name when creating the output file
	 * @exception IOException
	 *                if an error occurs
	 */
	public void compress(String infile) throws IOException {
		...
		mOut = new PrintWriter(new BufferedWriter(new FileWriter(infile
				+ ".lz77")));
		...
		while ((nextChar = mIn.read()) != -1) {
			...
			if (tempIndex != -1) {
				...
				String codedString = "~" + matchIndex + "~"
						+ currentMatch.length() + "~" + (char) nextChar;
	...
	public static void main(String[] args) {
		// if (args.length < 2) {
		// System.err.println("Usage: LZ77 [c|d] <filename>");
		// System.exit(1);
		// }
		args = new String[] { "d", "D:\\temp\\abc.txt" };
		LZ77 lz = new LZ77();
		try {
