...
public class MySemanticAnalyzer extends BaseSemanticAnalyzer {
  ...
  private List<ASTNode> doPhase1GetDistinctFuncExprs(
    ...
    for (Entry<String, ASTNode> entry : aggregationTrees.entrySet()) {
   ...
   * @see #handleInsertStatementSpec(List, String, RowResolver, RowResolver, QB, ASTNode)
   * @throws SemanticException
   */
  ...
  private String findAlias(ASTNode columnRef,
    ...
    if ( aliasToOpInfo != null ) {
      for (Entry<String, Operator> opEntry : aliasToOpInfo.entrySet()) {
  ...
  Integer genColListRegex(String colRegex, String tabAlias, ASTNode sel,
    ...
    for (String alias : aliases) {
      ...
      for (Entry<String, ColumnInfo> entry : fMap.entrySet()) {
  ...
  static String[] getColAlias(ASTNode selExpr, String defaultName,
    ...
    if (root.getType() == HiveParser.TOK_TABLE_OR_COL) {
      colAlias =
          unescapeIdentifier(root.getChild(0).getText());
      colRef[0] = tabAlias;
      colRef[1] = colAlias;
      return colRef;
    }
  ...
  private RowResolver handleInsertStatementSpec(List<ExprNodeDesc> col_list, String dest,
    ...
    if(partSpec != null) {
      ...
      for(Entry<String, String> partKeyVal : partSpec.entrySet()) {
  ...
  private ReduceSinkOperator genCommonGroupByPlanReduceSinkOperator(QB qb, List<String> dests,
    ...
    for (String destination : dests) {
      ...
      if (whereClause != null) {
        ...
        for (Entry<ASTNode, ExprNodeDesc> entry : nodeOutputs.entrySet()) {
  ...
  private Operator genBucketingSortingDest(String dest, Operator input, QB qb,
    ...
    if (enforceBucketing || enforceSorting) {
      ...
      for (int sortOrder : sortOrders) {
        order.append(sortOrder == HIVE_COLUMN_ORDER_ASC ? '+' : '-');
      }
  ...
  private Operator genUnionPlan(String unionalias, String leftalias,
    ...
    Iterator<Entry<String, ColumnInfo>> lIter = leftmap.entrySet().iterator();
    Iterator<Entry<String, ColumnInfo>> rIter = rightmap.entrySet().iterator();
    while (lIter.hasNext()) {
      Entry<String, ColumnInfo> lEntry = lIter.next();
      Entry<String, ColumnInfo> rEntry = rIter.next();
